...
function PortfolioOverview() {
  ...
  // Mobile nav header with portfolio switcher dropdown
  const activePortfolio = portfolios?.find((p) => p.id === selectedPortfolioId)

  const mobilePortfolioDropdown = useMemo(() => {
            ...
            >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-xs text-base-400">{p.ownerName}</div>
                </div>
              </button>
              <button
                type="button"
  ...
  useMobileDetailNav(
    {
      name: activePortfolio?.name ?? 'Investment Portfolio',
      subtitle: platformCountSummary ?? '',
      dropdown: mobilePortfolioDropdown ?? undefined,
    },
  ...
  return (
    <div className="relative z-0">
      ...
      >
        <Button variant="secondary" fullWidth onClick={() => setTransactionDialogOpen(true)}>
          + Add Transaction
        </Button>
        <Button variant="primary" fullWidth onClick={() => setDataPointDialogOpen(true)}>
          + Add Data Point
        </Button>
      </div>
